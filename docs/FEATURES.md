...
| W8 | Essential | **A token on the inbound side.** Raised Desired → Essential by Kenny. A profile with an HTTP source may require a shared token. Not urgent on the LAN, but it is the door that must exist before M4 can even be discussed. **Amended 2026-09-09 (H1, 3.0.0):** the token is no longer this service's own per-path `inbound_token` but a chassis client token, issued per sender and revocable in a second. The requirement is unchanged — a door with a credential — only its mechanism moved. |
...
| W8 | Without a token 401 and no delivery; with the right token normal handling; a revoked token 401 on the next request; the token appears in no log line, no refusal and no page. |
