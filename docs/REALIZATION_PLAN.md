...
| L8 | The two Desired features | K7, W4 | The dry-run command on the K12 fixture gives exactly the real path's output and demonstrably sends nothing | **done** |
...
|---|---|---|---|
| Phase 9 · release | 2026-08-30 | Kenny's explicit go. **v1.0.0 tagged on `main` (5f2e507)** and released. Verified by reading it back rather than assuming: the GitHub Release carries the binary and SHA256SUMS and is not a draft; an **anonymous** `docker pull` of `ghcr.io/kennypassenier/http-switchboard:1.0.0` succeeds; the downloaded binary's checksum verifies against the published manifest and the binary runs; the published image starts, answers `/healthz` and passes its own `--healthcheck` | CHANGELOG, GitHub Release |
